...
- SSISDB folder name. Ask the user; there is no default.
- Optional: parameter overrides as a hashtable (e.g. `@{ RunDate = '2025-01-15' }`).

Read `.ssis-toolkit.json` for `projectPath`, `projectName`, and `docsPath`.
...
7. **Report.** For each package: execution_id, status name, duration, rows affected (from `catalog.execution_data_statistics`), and a link to the matching `<docsPath>/<Pkg>.md` if present.

Refuse if:
- The package has not been generated yet (`<projectPath>/<Pkg>.dtsx` is missing).
