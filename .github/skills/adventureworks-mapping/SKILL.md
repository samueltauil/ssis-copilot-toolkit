...
There is no date dimension in the demo warehouse. `OrderDate` lands on the fact as a
degenerate date attribute — do not add a `dim.Date` lookup or a date surrogate key.
...
| via `Sales.SalesOrderHeader` | `OrderDate` | `OrderDate` (degenerate, no lookup) |
...
SELECT  d.SalesOrderID,
        ...
        h.CustomerID,
        d.ProductID,
        CAST(h.OrderDate AS datetime2(3)) AS OrderDate,
        d.OrderQty,
        d.UnitPrice,
        d.LineTotal
