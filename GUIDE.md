...
Everything below happens **in GitHub Copilot Chat**. You will not write PowerShell, you will not open `.dtsx` XML, you will not click through wizards. You type natural-language prompts into the Chat input; `@ssis-author` writes the metadata, calls the toolkit, and spawns `@ssis-validator` to prove the result. This guide shows you the prompts.
...
What `@ssis-author` does, in order:
...
4. Spawns `@ssis-validator`, which runs `Test-SsisPackage.ps1` (`dtexec /Validate /WarnAsError`) and `Test-SsisDesignerLoad.ps1` (`Application.LoadPackage` round-trip).
5. Returns a `VERDICT: PASS|FAIL` block with per-step exit codes.

You see all of it in the chat transcript. The agent never edits the `.dtsx` directly — only the metadata JSON. If the verdict is `FAIL`, the agent loads the `dtexec-validation-triage` skill, identifies which metadata field is wrong, patches the JSON, and re-runs the gate.
...
> Build dim.Customer from stg.Customer keyed on CustomerID, overwriting on key match.
```

```text
@ssis-author /generate-dim-type2-package
> Build dim.CustomerHistory from stg.Customer keyed on CustomerID. Track changes to FirstName, LastName, EmailAddress with IsCurrent + EffectiveFrom + EffectiveTo.
```

```text
@ssis-author /generate-fact-package
...
Anything that does not fit the four patterns? `@ssis-author` refuses and asks which pattern fits. That refusal is enforced by [.github/agents/ssis-author.agent.md](.github/agents/ssis-author.agent.md).

## Step 4 — Modify a package without touching XML or PowerShell

Just describe the change:

> `@ssis-author` change `Stg_Customer` so it does not truncate before load.
...
```text
@ssis-validator validate templates/ssis-project/Packages/Stg_Customer.dtsx
```

`@ssis-validator` is read-only. It cannot author, deploy, or execute — it only runs the gate and returns the verdict. The full procedure it follows lives in the [`ssis-delivery-gate`](.github/skills/ssis-delivery-gate/SKILL.md) skill.
...
`@ssis-author` emits T-SQL under `templates/sql/validation/` — row counts, key uniqueness checks, SCD-2 invariants (exactly one current row per business key, no overlapping effective dates), and source-to-target reconciliations. Run them in SSMS or with the `mssql` extension.
...
When those land, the chat experience is the same shape: a single prompt, the agent owns the work end-to-end, `@ssis-validator` (or the SSISDB equivalent) reports the verdict.
